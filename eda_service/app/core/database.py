...
async def init_db() -> None:
    ...
    try:
        ...
        logger.error("Ошибка инициализации базы данных: %s", e, exc_info=True)
        raise
...
async def close_db() -> None:
    ...
    try:
        ...
        logger.error("Ошибка при закрытии подключения к бд: %s", e, exc_info=True)
        raise
...
async def health_check_db() -> bool:
    try:
        ...
        logger.error("Health check бд не прошёл: %s", e)
        return False
