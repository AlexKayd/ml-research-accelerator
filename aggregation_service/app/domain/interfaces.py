...
class IDatasetRepository(ABC):
    ...
    @abstractmethod
    async def delete_file(self, dataset_id: int, file_name: str) -> bool:
        """Удаляет файл из датасета"""
        pass
    
    @abstractmethod
    async def add_file(self, dataset_id: int, file: File) -> bool:
        """Добавляет файл к существующему датасету"""
        pass

    @abstractmethod
    async def get_files_by_dataset(self, dataset_id: int) -> List[File]:
        """Получает список всех файлов датасета"""
        pass
    
    @abstractmethod
    async def recalculate_dataset_size(self, dataset_id: int) -> float:
...
class ISourceClient(ABC):
    ...
    @abstractmethod
    async def list_dataset_ids(self, batch_size: Optional[int] = None) -> List[str]:
    ...
    @abstractmethod
    async def get_files_list(self, external_id: str) -> List[dict]:
    ...
    @abstractmethod
    async def download_dataset(self, external_id: str, target_dir: str) -> str:
        """Скачивает датасет во временную директорию"""
        pass
    
    @abstractmethod
    async def check_dataset_exists( self, external_id: str) -> bool:
        """Проверяет существование датасета в источнике"""
        pass
    
    @abstractmethod
    def get_repository_url(self, external_id: str) -> str:
        """Формирует URL страницы датасета в репозитории"""
        pass
    
    @abstractmethod
    def get_download_url(self, external_id: str) -> Optional[str]:
