...
def _domain_to_http(exc: DomainException) -> tuple[int, str]:
    if isinstance(exc, DatasetNotFoundError):
        return status.HTTP_404_NOT_FOUND, "DATASET_NOT_FOUND"
    if isinstance(exc, SourceUnavailableError):
        return status.HTTP_503_SERVICE_UNAVAILABLE, "SOURCE_UNAVAILABLE"
    if isinstance(exc, (InvalidDatasetError, InvalidFileError)):
        return status.HTTP_422_UNPROCESSABLE_ENTITY, "VALIDATION_ERROR"
    if isinstance(exc, NoValidFilesError):
        return status.HTTP_422_UNPROCESSABLE_ENTITY, "NO_VALID_FILES"
    if isinstance(exc, DatasetTooLargeError):
        return status.HTTP_413_REQUEST_ENTITY_TOO_LARGE, "DATASET_TOO_LARGE"
    if isinstance(exc, (DatabaseError, HashCalculationError, AggregationError)):
...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    async def http_exception_handler(
        ...
        logger.warning("HTTPException: %s (url=%s method=%s)", exc.detail, request.url, request.method)
        ...
        return JSONResponse(
            ...
            content=_error_payload(
                code="HTTP_ERROR", message=message, details=details,
            ),
        )
